...
public class MySQLQueryGenerator implements QueryGenerator {
    ...
    public String generateInsertObjectQuery(Class<?> model, Object object) {
        ...
        sqlBuilder.append(fields.stream().map(field -> field.getType().equals(UUID.class) ? "UUID_TO_BIN(?)" : "?").collect(Collectors.joining(",")));
        sqlBuilder.append(");");
    ...
    public String generateSelectByID(Class<?> model, Object id) {
        ...
        sqlBuilder.append("SELECT * FROM ")
                ...
                .append(idColumnName)
                .append(" = ")
                .append(fieldId.getType().equals(UUID.class) ? "UUID_TO_BIN(?)" : "?")
                .append(" limit 1");
    ...
    public String generateUpdateQuery(Class<?> model, Object object) {
        ...
        sqlBuilder.append(fields.stream().filter(f -> !f.isAnnotationPresent(Id.class))
                .map(field ->
                        (!field.isAnnotationPresent(Column.class) || field.getAnnotation(Column.class).key().isBlank() ?
                                field.getName() :
                                DbConfig.getValue(field.getAnnotation(Column.class).key())) + " = " +
                                (field.getType().equals(UUID.class) ? "UUID_TO_BIN(?)" : "?"))
                .collect(Collectors.joining(", ")));
